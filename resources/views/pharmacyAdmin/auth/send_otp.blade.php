    ...
    <div class="d-flex flex-wrap align-items-stretch">
        <div class="col-lg-4 col-md-6 col-12 order-lg-1 min-vh-100 order-2 bg-white">
            <div class="p-4 m-3">
                ...
                @endphp
                @if(isset($app_logo->company_logo))
                <img src="{{ $app_logo->company_logo }}" alt="logo"  style="height: {{ $setting->nav_height }}px;"  class="mb-5 mt-2">
                @else
                        <img src="{{url('/images/upload_empty/logo_black.png')}}" class="h-6 mr-3 sm:h-9" alt=" Logo" />
                @endif
                @if (session('error'))
                ...
                <form action="{{ url('pharmacy_verify_otp') }}" method="post">
                    ...
                    <div class="digit-group mt-4" data-group-name="digits" data-autosubmit="false" autocomplete="off">
                ...
                <div class="text-muted text-center">
                    {{__("Don't Send A Code?")}}<a href="{{ url('pharmacy_send_otp') }}">{{__('Send Code Again')}}</a>
                </div>
